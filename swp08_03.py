...
if __name__ == '__main__':
    ...
    # (values entered here are -1 from the values entered in Calrec UI)
    test1 = Message.connect(10, 1)   
    ...
    while connection.status == "Not Connected":
        print(connection.status)
        
    
    # mute/disconnect anything 
    print("sending message - connect source 3")
    connection.send(test3._encoded)
    time.sleep(2)
    # patch tone
    print("sending message - connect source 1")
    connection.send(test1._encoded)
    time.sleep(2)
    #path audio
    print("sending message - connect source 2")
