...
def create_labels(router, matrix, level, io_type, label_type):
    ...
    if io_type == 'destination':
        io = router.destinations
    elif io_type == 'source':
        io = router.sources
    else:
        print("[connectIO_gui.create_labels]: invalid io_type: {}".format(io_type))
        return False
...
def create_cross_point_buttons(router, matrix, level):
    ...
    for dest in router.destinations:
        if dest.matrix == matrix and dest.level == level:
            sources = []
            for src in router.sources:
                # TODO - LABEL IS FOR DEBUG, REMOVE LABEL ONCE DEBUGGED!
                if src.matrix == matrix and src.level == level:
                    btn = CrossPointButton(str(dest.id) + "-" + str(src.id), src, dest)
                    btn.setCheckable(True)
                    btn.setProperty('cssClass', ['cross_point'])
                    tooltip = create_cross_point_tooltip(src, dest)
                    btn.setToolTip(tooltip)
...
def create_cross_point_grid(source_labels, source_user_labels, source_id_labels, source_external_labels,
    ...
    for label in source_user_labels:
        print("DEBUG user label", label.text())
        layout.addWidget(label, row, col)
        row += 1
    ...
    for label in source_labels:
        print("DEBUG source label", label.text())
        layout.addWidget(label, row, col)
        row += 1
...
class MessageItem(QWidget):
    def __init__(self, message, count=0):
        ...
        if count:
            message_field.setStyleSheet(CSS['message even'])
            timestamp_field.setStyleSheet(CSS['message even'])
        else:
            message_field.setStyleSheet(CSS['message odd'])
            timestamp_field.setStyleSheet(CSS['message odd'])

        if message.command == "ACK":
            message_field.setStyleSheet(CSS['ACK'])
            timestamp_field.setStyleSheet(CSS['ACK'])

        if message.command == "NAK":
            message_field.setStyleSheet(CSS['NAK'])
            timestamp_field.setStyleSheet(CSS['NAK'])
...
class MessagingView(QWidget):
    ...
    def __init__(self, parent):
        ...
        self.received_widget = QWidget()

        self.sent_scroll_area = QScrollArea()
        self.received_scroll_area = QScrollArea()

        self.sent_scroll_area.setWidget(self.sent_widget)
        self.received_scroll_area.setWidget(self.received_widget)
        ...
        #scroll.setWidget(self.sent_window)
        #self.setCentralWidget(scroll)

        #scroll = QScrollArea()
        # scroll.setWidgetResizable(True)
        #scroll.setWidget(self.received_window_window)
        # self.setCentralWidget(scroll)
    ...
    def add_received(self, message):
        item = MessageItem(message, self.received_window.count())
        self.received_window.addWidget(item)


    def add_sent(self, message):
        item = MessageItem(message, self.sent_window.count())
...
def refresh_cp_view(parent):
    ...
    #print("[connectIO_gui.refresh_cp_view]: refreshing cp grid")
...
class MainWindow(QMainWindow):
    ...
    def update_status(self):
        self.statusBar.setStyleSheet(CSS[self.router.connection.status])
    ...
    def refresh(self):
        ...
        for dest in self.cross_point_columns:
            ...
            connected_source = dest[0].destination.connected_source
            for cp in dest:
