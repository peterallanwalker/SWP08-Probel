...
def decode(encoded_message):
    ...
    if encoded_message == bytes(utils.ACK):
        ...
        if command in ('connect', 'connected'):
            ...
            tallies = encoded_message[utils.COMMAND_BYTE + 2]
            
            #print("DEBUG SWP TALLY DUMP MESSAGE, number of tallies:", tallies)
            ...
            #    div = encoded_message[src_byte]
            #    mod = encoded_message[src_byte + 1]
            #    sources.append(256 * div + mod)
            #    src_byte += 2
            
            src = 0
            while src < tallies:
                #print("tally", src)
                div = encoded_message[utils.COMMAND_BYTE + 5 + src]
                mod = encoded_message[utils.COMMAND_BYTE + 6 + src]
                sources.append(256 * div + mod)
                src += 1
...
class CrossPointTallyDumpWord:
    def __init__(self, first_destination, connected_sources):
        if len(connected_sources) > 64:
            ...
            self.verbose = self.verbose_listing()
            self.encoded = self._encode()
    ...
    def verbose_listing(self):
        r = ''
        for i, src in enumerate(self.sources):
            r += f'Destination {self.destination + i} <- Source {src}\n'
        return r
