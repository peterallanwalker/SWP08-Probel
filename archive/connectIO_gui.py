...
def create_source_label_col(sources, grid_layout, label='label', row=2, col=1):
    ...
    :param sources: Dict of sources, as supplied by import_io's sources{matrix{level{id{data}}}}
    :param grid_layout: A pyQt QGridLayout
    :param label: Str - label to use, default='label', or user-label 'ulabel'
...
class MainWindow(QMainWindow):
    def __init__(self, router):
        ...
        grid_layout.addWidget(QLabel("Sources"), 2, 1, 1, 2)
        # - Apply destination label row header, spanning 2 rows (3rd arg)
        grid_layout.addWidget(VerticalLabel("Destinations"), 0, 3, 2, 1)

        # - Create source/dest labels for Matrix 1, Level 1
        create_source_label_edit_col(self.router, grid_layout, row=2, col=0)
        create_source_label_col(self.router.io['sources']['1']['1'], grid_layout, label='ulabel', row=2, col=1)
        create_source_label_col(self.router.io['sources']['1']['1'], grid_layout, label='label', row=2, col=2)
        create_destination_labels(self.router.io['destinations']['1']['1'], grid_layout, label='ulabel', row=0, col=3)
        create_destination_labels(self.router.io['destinations']['1']['1'], grid_layout, label='label', row=1, col=3)

        # - Create the cross-point grid
        self.cross_points = create_cross_points(self.router, grid_layout, row=3, col=4)
